...
function bindEvent2(ele, type, fn, selector) {
  ...
  ele.addEventListener(type, function(event){
  	//通过以下代码,可以获取到触发当前事件的触发者(也就是事件流程中的最内层元素)
    const target = event.target
    if (selector) {
    	//可以判断target是否满足selector选择器的要求,满足返回true,不满足返回false
      if (target.matches(selector)) {
        fn.call(target, event)
      } 
    } else {
    //.call的目的就是为了改变fn函数的this指向,同时传入实参event,防止用户传入的fn回调函数中,使用到this和event
      fn.call(ele, event)
    }
  })
}
...
- 2XX: 表示成功处理请求,例如:
  - 200(完全成功,完美)
  - 201(本次请求成功,但是传递的参数不对)
- 3XX: 需要重定向, 浏览器直接跳转,例如:
  - 302(用户请求/a接口,服务器将本次请求返回,并引导浏览器请求/b接口),
  - 304(协商缓存:本次请求的资源,上次已经请求过了,目前还在你的浏览器内存中,你去使用上次的资源,相当于把当前请求重定向到用户的浏览器内存中)
- 4XX: 客户端请求错误
  - 403(代表服务器有这个资源,但是你的权限不够,拒绝你的请求)
    - 大概率就是当前的帐号权限不够,找后端
  - 404(代表服务器没有该资源,资源找不到,对于服务器来说,接口也是他们的资源)
    - 可能出现的原因:
      - 1.我们路径写错了
      - 2.后端还没有实现这个接口
- 5XX: 服务器端错误,例如:
  - 500(代表服务器逻辑出错,服务器无法正常运行)
  - 509(代表当前资源永久更换了地址)

#### 区别ajax请求与一般HTTP请求

ajax不是一种特殊的请求,他是一种发送请求的技术,可以用ajax发送请求,实际上发出去的都是http请求
...
1. 配置通用的基础路径和超时时间
   1. 问题1:为什么需要配置基础路径
      1. 为了解决跨域报错,我们使用devServer实现请求的转发,添加基础路径就是为了配合proxy规则,告知开发服务器当前请求需要转发
   2. 问题2:为什么要配置超时时间?
      1. 由于Chrome和Firefox浏览器,他们的最大请求并发数是6个,所以必须配置超时时间
      2. 否则就可能出现,多个请求通道被长时间占用,导致能使用的请求通道数量越来越少,后续请求越来越慢,拖慢了页面的显示速度
2. 显示请求进度条
   1. 显示进度条: 请求拦截器回调
   2. 结束进度条: 响应拦截器回调
...
1. 使用localStorage实现跨标签页通信
   1. 流程:
      1. 首先在B页面给window对象绑定storage事件的监听
      2. 当A页面存入localStorage数据的时候,B页面的storage事件的回调函数就会触发
         1. 在该回调函数的event对象中,可以获取到本轮存储的数据
2. 使用url路径实现跨标签页通信
   1. 流程:
      1. A页面使用window.location.href=地址跳转页面时,可以在路径中拼接参数
      2. B页面显示的时候,可以通过window.location中获取到当前路径中的相关参数
3. 使用domain属性实现跨标签页通信效果
   1. 流程:
      1. B页面通过window.open方法打开A页面,此时open方法会返回A页面的window对象
      2. A页面执行document.domain="二级域名"
      3. B页面执行document.domain="二级域名"
      4. A页面在自己的window对象身上添加数据
      5. B页面通过流程1中获得的window对象,可以直接读取到A页面的数据
      6. 说明:
         1. 流程2和3的目的,就是为了让A和B页面的数据可以实现互相传递效果
